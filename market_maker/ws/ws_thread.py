...
class BitMEXWebsocket():
    ...
    def __wait_for_account(self):
        ...
        conn_timeout = 50
        while not {'margin', 'position', 'order'} <= set(self.data) and conn_timeout:
            sleep(0.1)
            conn_timeout -= 1

        if not conn_timeout or self._error:
            self.logger.error("Account data not received in 5 seconds via WS! Exiting.")
            self.exit()
            sys.exit(1)
    ...
    def __wait_for_symbol(self, symbol):
        '''On subscribe, this data will come down. Wait for it.'''
        conn_timeout = 50
        while not {'instrument', 'trade', 'quote'} <= set(self.data) and conn_timeout:
            sleep(0.1)
            conn_timeout -= 1

        if not conn_timeout or self._error:
            self.logger.error("Symbol data not received in 5 seconds via WS! Exiting.")
            self.exit()
            sys.exit(1)
